...
--? No sé si esté bien que la tabla se autoreferencíe
...
-- Crear las tablas SUPPLIERS y PROJECTS sin la relación PROJECT_ID y luego actualizar:

ALTER TABLE SUPPLIERS
    ADD CONSTRAINT SUPPLIERS_FK_PROJECTS FOREIGN KEY (
        PROJECT_ID
    )
        REFERENCES PROJECTS(
            PROJECT_ID
...
CREATE TABLE PROJECT_ASIGNMENT (
    ...
    -- Resulta que 'ROLE' es palabra reservada para cosas de manejo de la base de datos.
    ROLL VARCHAR2(20),
    HOURS_WORKED NUMBER,
    FOREIGN KEY (PROJECT_ID) REFERENCES PROJECTS(PROJECT_ID),
...
-- ? Ya que la PK de esta tabla es una tupla no sé si esto afecte el ejercicio 10 en donde pide info de esta misma tabla.
