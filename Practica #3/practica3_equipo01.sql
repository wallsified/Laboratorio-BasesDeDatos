...
--
-- El cálculo del aumento de salario se hace usando sumas y multiplaciones y luego se hace el SET y el WHERE
-- para actualizar solo los que sean Managers. 
...
--
-- Se seleccionan todos los employee_id de Employees y luego los de Project_Assignments. Los que no 
-- aparezcan en Project_Assignments pero si en Employees se eliminan de Employees.
...
-- 
-- Investigamos como poner "alias" para las tablas sin usar el comando AS. Esto se hace despues de 
-- declarar el FROM se pueden empezar a ocupar desde antes. Por ejemplo, hacemos d.departament_name
-- y luego del comando hacemos FROM Departments d.

SELECT d.department_name, employee_count FROM Departments d,
     (SELECT department_id, COUNT(*) as employee_count FROM Employees
      GROUP BY department_id HAVING COUNT(*) > 10) e
...
--
-- Basta con ocupar 'DISTINCT' y 'LIKE' para poder hacer el filtro en la tabla Employees.
SELECT DISTINCT fst_name, lst_name FROM Employees WHERE email LIKE '%@example.com';

-- 11. Muestra los nombres de los empleados que están trabajando en más de un proyecto. Ordena los
-- resultados alfabéticamente.
-- 
-- Hacemos una consulta secundaria para agrupar los proyectos que tengan más de un employee_id 
-- asignado. Luego ordenamos por nombre los ID que aparezcan en el filtro. 
SELECT fst_name, lst_name FROM Employees WHERE employee_id IN (
    SELECT employee_id FROM Project_Assignments GROUP BY employee_id HAVING COUNT(*) > 1)
...
--
-- Lo que se hace es hacer una consulta secundaria para obtener el total de horas asignadas de un proyecto de 
-- la tabla Project_Assignments y luego se filtra para solo considerar aquellos proyectos que tengan más de 
-- 1000 horas asignadas. Finalmente lo que se hace es devolver los nombres y las horas asignadas de aquellos ID
-- que coincidan con los que aparecen en la consulta secundaria.
SELECT p.project_name, total_hours FROM Projects p,
     (SELECT project_id, SUM(hours_allocated) as total_hours FROM Project_Assignments
      GROUP BY project_id HAVING SUM(hours_allocated) > 1000) pa
